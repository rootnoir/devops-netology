...
```
AWS не доступен с территории РФ.
Далее рассматриваю только задания по YC
```
...
```shell
root@vagrant:~/terraform# touch {main,versions}.tf
root@vagrant:~/terraform# ll
total 8
drwxr-xr-x  2 root root 4096 Jun 17 18:53 ./
drwx------ 23 root root 4096 Jun 17 18:52 ../
-rw-r--r--  1 root root    0 Jun 17 18:53 main.tf
-rw-r--r--  1 root root    0 Jun 17 18:53 versions.tf
...
```shell
root@vagrant:~/terraform# cat ~/.terraformrc
...
root@vagrant:~/terraform# cat versions.tf 
terraform {
  required_providers {
    yandex = {
...
```
```
root@vagrant:~/terraform# terraform init
...
```shell
yc compute image list --folder-id standard-images | grep 'ubuntu-2004-lts\s'
```
...
```
Использовал инструкцию от yandex
```
[Terraform QuickStart](https://cloud.yandex.ru/docs/tutorials/infrastructure-management/terraform-quickstart)
...
```shell
root@vagrant:~/terraform# terraform plan
...
Changes to Outputs:
  ...
  + zone-vm1                 = (known after apply)
  + zone-vm2                 = (known after apply)
